...
class TD3(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100):
        ...
        for _ in range(gradient_steps):
            ...
            # TODO, can get sampled_action from scaled action rather than a different line
            ...
            with th.no_grad():
                ...
                scaled_next_action = self.consistency_model.batch_multi_sample(model=self.actor, state=next_state_rpt) # 256, 50, 17
                ...
                next_q_values = next_q_values + huber_coef * next_huber_distance # TODO, is so small here, where is about -10 in SAC
        ...
        if len(huber_coef_losses) > 0:
            self.logger.record("train/huber_coef_loss", np.mean(huber_coef_losses))
